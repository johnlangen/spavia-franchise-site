...
export async function POST(req: Request) {
  try {
    ...
    // 2.5 Remove the "Incomplete Lead" tag if present. This lead just
    // completed the long form, so the partial-capture tag (applied at step 1)
    // is now stale. Same tag id as acPartial.
    try {
